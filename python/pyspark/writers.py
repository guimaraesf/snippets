...
class PysparkWriters:
    ...
    def write_csv_file(
        ...
        if dest_path.endswith(".csv"):
            df.coalesce(PARTITIONS).write.mode(mode).options(
                delimiter=delimiter, encoding=encoding, header=header, maxRecordsPerFile=MAX_RECORDS_PER_FILE
            ).csv(dest_path)
    ...
    def write_parquet_file(
        ...
        if dest_path.endswith(".parquet"):
            df.coalesce(PARTITIONS).write.mode(mode).options(maxRecordsPerFile=MAX_RECORDS_PER_FILE).parquet(
                dest_path
            )
        # Unpersist the DataFrame to free up memory
        df.unpersist()
