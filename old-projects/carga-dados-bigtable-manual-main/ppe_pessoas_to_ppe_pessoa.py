...
def run():
    bigtable_instance_origin = BigTableClient(project_id_hml, instance_id_hml).get_instance()
    ...
    for prefix in prefixes:
        ...
        if bool(rows):
            for row in rows:
                ...
                print('Writing dev')
                WriteRowBigTableV2(table_destiny_dev).write_row(changed_row[0], changed_row[1])
                print('Writing hml')
                WriteRowBigTableV2(table_destiny_hml).write_row(changed_row[0], changed_row[1])

    else:
        print('Empty origin')


def change_row(input_data: tuple[str, dict[str, dict[str, str]]]) -> tuple[str, dict[str, dict[str, str]]]:
