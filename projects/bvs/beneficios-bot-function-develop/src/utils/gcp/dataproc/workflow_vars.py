...
class WorkflowVariables:
    ...
    def __init__(
        self, env_vars=function_env_vars, list_of_files=Variables.PYTHON_FILES
    ) -> None:
        ...
        self.bucket_init_actions_name = self.env_vars.get(
        ...
        self.table_name_garantias_safra = self.env_vars.get(
        ...
        self.table_name_reserva_ref_militares = self.env_vars.get(
        ...
        self.table_name_fornecedores_compras = self.env_vars.get(
            "TABLE_NAME_FORNECEDORES_COMPRAS"
        )
        self.table_name_fornecedores_item_compra = self.env_vars.get(
            "TABLE_NAME_FORNECEDORES_ITEM_COMPRA"
        )
        self.table_name_fornecedores_termo_aditivo = self.env_vars.get(
        ...
        self.table_name_cnpj_estabelecimentos = self.env_vars.get(
            "TABLE_NAME_CNPJ_ESTABELECIMENTOS"
        )
        self.table_name_cnpj_imune_isentas = self.env_vars.get(
            "TABLE_NAME_CNPJ_IMUNE_ISENTAS"
        )
        self.table_name_cnpj_lucro_arbitrado = self.env_vars.get(
            "TABLE_NAME_CNPJ_LUCRO_ARBITRADO"
        )
        self.table_name_cnpj_lucro_presumido = self.env_vars.get(
            "TABLE_NAME_CNPJ_LUCRO_PRESUMIDO"
        )
        self.table_name_cnpj_lucro_real = self.env_vars.get(
            "TABLE_NAME_CNPJ_LUCRO_REAL"
        )
        self.table_name_cnpj_motivos = self.env_vars.get("TABLE_NAME_CNPJ_MOTIVOS")
        self.table_name_cnpj_municipios = self.env_vars.get(
            "TABLE_NAME_CNPJ_MUNICIPIOS"
        )
        self.table_name_cnpj_natureza_juridica = self.env_vars.get(
            "TABLE_NAME_CNPJ_NATUREZA_JURIDICA"
        )
        self.table_name_cnpj_paises = self.env_vars.get("TABLE_NAME_CNPJ_PAISES")
        self.table_name_cnpj_qualificacao_socio = self.env_vars.get(
        ...
        self.table_name_debitos_nao_prev = self.env_vars.get(
        ...
        self.table_name_sancoes_acordos = self.env_vars.get(
            "TABLE_NAME_SANCOES_ACORDOS"
        )
        self.table_name_sancoes_efeitos = self.env_vars.get(
            "TABLE_NAME_SANCOES_EFEITOS"
        )
        self.table_name_servidores_df_desp_emp = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_EMPENHO"
        )
        self.table_name_servidores_df_desp_emp_desc = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_EMPENHO_DESC"
        )
        self.table_name_servidores_df_desp_emp_sub = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_EMPENHO_SUB"
        )
        self.table_name_servidores_df_desp_lanc = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_LANCAMENTO"
        )
        self.table_name_servidores_df_desp_nt_lanc = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_NT_LANC"
        )
        self.table_name_servidores_df_desp_nt_lanc_evento = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_NT_LANC_EVENTO"
        )
        self.table_name_servidores_df_desp_ord_canc = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_ORD_CANCELADA"
        )
        self.table_name_servidores_df_desp_ord_emp = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_ORD_EMPENHO"
        )
        self.table_name_servidores_df_desp_pgto = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_PGTO"
        )
        self.table_name_servidores_df_desp_principal = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_DESP_PRINCIPAL"
        )
        self.table_name_servidores_df_licitacao = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_LICITACAO"
        )
        self.table_name_servidores_df_orgao = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_ORGAO"
        )
        self.table_name_servidores_df_patrimonio = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_PATRIMONIO"
        )
        self.table_name_servidores_df_receita = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_RECEITA"
        )
        self.table_name_servidores_df_remuneracao = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_REMUNERACAO"
        )
        self.table_name_servidores_df_remuneracao_det = self.env_vars.get(
            "TABLE_NAME_SERVIDORES_DF_REMUNERACAO_DET"
        )
        self.python_file_uris = [
            f"gs://{self.bucket_jobs_name}/{py_file}" for py_file in self.list_of_files
        ]
