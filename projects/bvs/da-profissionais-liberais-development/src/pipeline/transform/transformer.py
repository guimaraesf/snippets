...
ITRANSFORM_MOD_PATH = get_module_path("src.interfaces.interface_transformer")
...
ITransformer = importlib.import_module(ITRANSFORM_MOD_PATH).ITransformer


class Transformer(ITransformer, SchemaTables, CleanTables, PysparkHandler):
    """
    A class that transforms data using schemas and tables, handles PySpark operations, and cleans tables.
    """
    ...
    def __init__(
        ...
        self.staging_layer = self.variables.STAGING_NAME
    ...
    def process_all_blobs(
        ...
        Args:
            ...
            table_infos (dict): A dictionary containing information about the tables to which the blob data will be loaded.
    ...
    def check_before_processing(self, blob_names: list):
        """
    ...
    def run_transform(
        ...
        if self.check_before_processing(blob_names):
            ...
            self.process_all_blobs(spark, bucket_id, blob_names, table_infos, df_params)
